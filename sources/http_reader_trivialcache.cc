...
#include "http_reader_blockcache.h"  // for RangeInfo
#include <curl/curl.h>
#include <cstring>
#include <cstdio>
#include <algorithm>
...
// --- HttpReaderTrivialCacheImpl helper ---

bool HttpReaderTrivialCacheImpl::fetch_range(uint64_t start, uint64_t end, std::vector<uint8_t>& out_data)
{
...
// --- heif_reader callbacks (userdata is HttpReaderTrivialCacheImpl*) ---

static int64_t cb_get_position(void* userdata)
{
  HttpReaderTrivialCacheImpl* impl = static_cast<HttpReaderTrivialCacheImpl*>(userdata);
...
// --- HttpReader_TrivialCache class implementation ---

HttpReader_TrivialCache::HttpReader_TrivialCache()
  : m_impl(std::make_unique<HttpReaderTrivialCacheImpl>())
{
...
bool HttpReader_TrivialCache::init(const char* url)
{
  m_impl->url = url;
  m_impl->file_size = -1;
...
void HttpReader_TrivialCache::cleanup()
{
...
const heif_reader* HttpReader_TrivialCache::get_heif_reader() const
{
  return &m_heif_reader;
}

void* HttpReader_TrivialCache::get_callback_user_data() const
{
  return m_impl.get();
}

int64_t HttpReader_TrivialCache::get_file_size() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->file_size;
}

std::vector<RangeInfo> HttpReader_TrivialCache::get_cached_ranges() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
